...
void main() {
  test('parses CSV with header', () {
    final file = File('/tmp/test_import_csv_header.csv');
  ...
  test('parses CSV without header', () {
    final file = File('/tmp/test_import_csv_noheader.csv');
  ...
  test('skips empty rows', () {
    final file = File('/tmp/test_import_csv_empty.csv');
    file.writeAsStringSync('tibetan,pronunciation,translation\n'
        '\n'
        'བཀྲ་ཤིས།,zha xi,\n'
        '   ,,\n'
  ...
  test('parses TSV', () {
    final file = File('/tmp/test_import_tsv.tsv');
    file.writeAsStringSync('tibetan\tpronunciation\ttranslation\n'
        'བཀྲ་ཤིས།\tzha xi\tgood luck\n');
    final result = BatchImportService.parseFile(file);
  ...
  test('handles alternate header names', () {
    final file = File('/tmp/test_import_alt_header.csv');
    file.writeAsStringSync('tib,pron,trans\n'
        'བཀྲ་ཤིས།,zha xi,good luck\n');
    final result = BatchImportService.parseFile(file);
    expect(result.importedRows, 1);
  });
