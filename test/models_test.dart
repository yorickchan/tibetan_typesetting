...
void main() {
  group('MarginMm', () {
    test('copyWith preserves unchanged fields', () {
      const original = MarginMm(top: 10, right: 20, bottom: 30, left: 40);
  ...
  group('PageSetup', () {
    ...
    test('titlePageTemplateId round-trip (non-null)', () {
      final original = PageSetup(titlePageTemplateId: 'template-uuid-123');
      final json = original.toJson();
      expect(json['titlePageTemplateId'], 'template-uuid-123');
      final restored = PageSetup.fromJson(json);
      expect(restored.titlePageTemplateId, 'template-uuid-123');
    });

    test('titlePageTemplateId round-trip (null)', () {
      final original = PageSetup();
      final json = original.toJson();
      expect(json.containsKey('titlePageTemplateId'), false);
      final restored = PageSetup.fromJson(json);
      expect(restored.titlePageTemplateId, null);
    });

    test('titleTextInset round-trip', () {
      final original = PageSetup(
      ...
      final restored = PageSetup.fromJson(json);
      expect(restored.titleTextInset.top, 21);
      expect(restored.titleTextInset.right, 32);
      expect(restored.titleTextInset.bottom, 23);
      expect(restored.titleTextInset.left, 34);
    });
  });
